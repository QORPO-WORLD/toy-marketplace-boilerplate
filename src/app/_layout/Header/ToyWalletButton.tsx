...
function ToyWalletBtn() {
  ...
  return (
    <>
        ...
        {isOpen && (
          <div className="absolute top-[99%] right-0 w-full bg-white h-fit rounded-b-[2.6875rem] tooltip overflow-hidden">
            <ul className="text-black">
              ...
              {isConnected && (
                <li
                  className="flex items-center gap-2 px-6 py-4 hover:bg-opacity-black"
      ...
      {netWorkSwitchModal && (
        <ModalLayout onClose={() => setNetworkSwitchModal(false)}>
          <NetworkSelectModalContent
            onClose={() => setNetworkSwitchModal(false)}
          />
        </ModalLayout>
      )}
      {logoutModal && (
        <ModalLayout onClose={() => setLogoutModal(false)}>
          <WalletModalContent />
        </ModalLayout>
      )}
    </>
